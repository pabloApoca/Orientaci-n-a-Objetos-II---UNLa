...
public class ClienteDao {
	...
	public List<Cliente> traerCliente() throws HibernateException {
		...
		try {
			iniciaOperacion();
			lista = session.createQuery("from Cliente c order by c.idCliente").list();
		} finally {
			session.close();
		}
		return lista;
	...
	@SuppressWarnings("unchecked")
	public List<Cliente> traerCliente2(long idInscripcion) throws HibernateException {
		List<Cliente> lista = null;
		try {
			iniciaOperacion();
			lista = session.createQuery("from Cliente c inner join fetch c.inscripcionAfip i where i.idInscripcionAfip="+idInscripcion).list();
		} finally {
			session.close();
		}
		return lista;
